defmodule PensieveFSMTest do
  use ExUnit.Case, async: false
  use PropCheck
  import PropCheck.FSM

  property "pensieve FSM operations", [:verbose, {:numtests, 100}, {:constraint_tries, 50}] do
    forall(cmds <- commands(PensieveFSM)) do
      {:ok, apps} = Application.ensure_all_started(:pensieve)
      {history, state, result} = run_commands(PensieveFSM, cmds)
